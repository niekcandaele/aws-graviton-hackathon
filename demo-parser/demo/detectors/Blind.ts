...
export default class Blind extends Detector {
  ...
  async calculate(): Promise<void> {
    this.demoFile.gameEvents.on('player_blind', async (e) => {
      ...
      blindRecord.attacker = await createPlayerInfo(this.demoFile, attacker, 'player_blind', this.currentRound);
      blindRecord.attacker.player = this.findMatchingPlayer(attacker);
      blindRecord.victim = await createPlayerInfo(this.demoFile, victim, 'player_blind', this.currentRound);
